...
pgsql_actors([{{select, Size}, Rows, _OpMeta}], Meta) ->
    Actors = [
        ...
        || {UID, Namespace, Group, Vsn, Res, Name, {jsonb, Data}, {jsonb, MetaData}, Hash} <- Rows
    ],
...
pgsql_actors_id([{{select, Size}, Rows, _OpMeta}], Meta) ->
    Actors = [
        ...
        || {UID, Namespace, Group, _Vsn, Res, Name, _Updated} <- Rows
    ],
...
pgsql_totals_actors_id([{{select, 1}, [{Total}], _}, {{select, Size}, Rows, _OpMeta}], Meta) ->
    ...
    Last = case lists:reverse(Rows) of
        [{_UID, _Namespace, _Group, _Vsn, _Res, _Name, Updated}|_] ->
            Updated;
        [] ->
            undefined
    end,
...
%% @private
make_sql_filters(#{domain:=Namespace}=Params) ->
    ...
    PathFilter = list_to_binary(filter_path(Namespace, Params)),
